  ...
  return (
    <Fragment>
      <Box style={style} className={classes.root}>
        ...
        {Memo && (
          <Box sx={{ fontSize: '15px' }}>
            ...
            <Typography component={'h5'} variant="caption" sx={{ color: 'black', mt: 0.2, textAlign: 'center', fontSize: '15px', width: '223px' }}>
              <span style={{ color: 'blue' }}>Click to Upload</span> Or drag & drop
            </Typography>
            <Typography component={'h5'} variant="caption" sx={{ color: 'black', mt: 0.2, textAlign: 'left', fontSize: '15px' }}>
              (Max. File size: 10 MB)
            </Typography>
          </Box>
        )}
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: '2px', flexWrap: 'wrap' }}>
        {false ? <CircularProgress /> : fileAvailable?.length > 0 && fileAvailable.map((item, index) => (
          <Box key={index} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }} >
            {item?.extension &&
              <Box sx={{ position: 'relative' }}>
                ...
                <Box component={'img'} sx={{cursor:'pointer'}} onClick={()=>{console.log(item);
                ...
                <p  onClick={()=>{console.log(item);
                 window.open(import.meta.env.VITE_BASE_URL + item.path, '_blank')}} style={{ color: 'blue',width:"120px" ,cursor:'pointer'}}>{item?.name}</p>
