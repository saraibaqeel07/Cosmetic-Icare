...
const UpdateEvent = () => {
    ...
    const UpdateEvent = async () => {
        ...
        try {
            ...
            const promise = ApiServices.UpdateEvent(obj);
    ...
    useEffect(() => {
        ...
        if (state) {
            setUploadedImages(state?.photos)
            setValue('name', state?.name)
            setValue('address', state?.address)

            setValue('starttime', state?.start_time)
            setValue('endtime', state?.end_time)
    ...
    return (
        <div>
            <Paper sx={{ width: "100%", overflow: "hidden", boxShadow: 'none' }}>
                ...
                <Box component={'form'} onSubmit={handleSubmit(UpdateEvent)} >
                    {console.log(options)}
