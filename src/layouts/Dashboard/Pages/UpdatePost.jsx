...
const UpdatePost = () => {
    ...
    const getData = async () => {
        try {
            ...
            const updatedTopics = data?.data?.topics?.map((item) => ({
                ...item,
                name: item.title,
            }));
    ...
    const getPostData = async () => {
        try {
            ...
            const data = await ApiServices.getPostData(params);

            setPostData(data?.data?.post)
            setValue('title',data?.data?.post?.title)
            setValue('description',data?.data?.post?.description)
            let selected=topics.find(item => item?._id == data?.data?.post?.topic_id)
            console.log(selected);
            setValue('topic',selected)
            setValue("image", { shouldValidate: true });
            
            setSelectedTopic(selected)
            setUploadedImages([data?.data?.post?.image])
    ...
    const UpdatePost = async () => {
        ...
        try {
            ...
            const promise = ApiServices.UpdatePost(obj);
            ...
            if (response?.responseCode === 200) {
                navigate("/posts"); // Change to your desired route
            }
