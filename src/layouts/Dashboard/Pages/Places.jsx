...
const Places = () => {
    ...
    const getData = async () => {
        try {
            ...
            const data = await ApiServices.getPlaces(params);
...

        {
            ...
            cell: ({ row }) => (
                <Box variant="contained" color="primary" sx={{ cursor: 'pointer', display: 'flex' }} >
                    ...
                    <Chip sx={{ backgroundColor: row?.original?.type == "Restaurant" ? '#56bf56' : '#5f64fd', color: 'white', width: '100px' }} label={row?.original?.type == 'Club' ? 'Bar/Club': 'Restaurant'} />
                </Box>
            ),
        },

        {
            ...
            cell: ({ row }) => (
                ...
                <Box variant="contained" color="primary" sx={{ cursor: 'pointer', display: 'flex', gap: 2 }} >
                    ...
                    <IconButton onClick={() => navigate("/update-place", { state: row?.original })}>
        ...
        <div>
            ...
            <Paper sx={{ width: "100%", overflow: "hidden", boxShadow: 'none' }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <PrimaryButton onClick={() => navigate('/create-place')} title={"Create"} />
                </Box>
