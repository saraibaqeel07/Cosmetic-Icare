import { Box, Chip, IconButton, Paper, Switch, Typography } from "@mui/material"
          ...
          },
          {
            ...
            cell: ({ row }) => {
              ...
              const handleToggle = async () => {
                ...
                try {
                  const obj = {
                    _id: row.original._id,   // assuming _id is available in row.original
                    is_active: newStatus ? true : false // depends on your backend (1/0 or true/false)
                  };
...
,          
      
        {
            ...
            cell: ({ row }) => (
                ...
                <Box variant="contained" color="primary" sx={{ cursor: 'pointer', display: 'flex', gap: 2 }} >
                 <IconButton onClick={() => navigate(`/form-template-detail/${row?.original?._id}`)}>
                        <NorthEastIcon sx={{ fontSize: '16px' }} />
                    </IconButton>
                    <IconButton onClick={() => navigate(`/update-form-template/${row?.original?._id}`)}>
                ...
                <Box sx={{ p: 2 }}>
                    ...
                    <Typography variant="h5" sx={{ mt: 4,mb:4, fontWeight: 600 }}>
                     Form Templates
                        ...
                        <PrimaryButton onClick={() => navigate('/create-form-template')} title={"Create"} />
                    </Box>
